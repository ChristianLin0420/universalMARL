...
class RolloutWorker:
    ...
    def rollout(self, env, ret, train=True, random_rate=0.):
        ...
        iterations = 24
        T_rewards, T_wins = np.zeros((env.n_threads, iterations)), np.zeros((env.n_threads, iterations))
            ...
            while True:
                ...
                for n in range(env.n_threads):
                    if not episode_dones[n]:
                        T_rewards[n][idx] = np.mean(rewards[n])
                        if np.all(dones[n]):
                            episode_dones[n] = 1
                            if infos[n][0]['won']:
                                T_wins[n][idx] = 1.
        ...
        T_rewards = T_rewards.flatten()

        self.model.train(True)
        self.critic_model.train(True)
        
        return T_rewards, T_wins
