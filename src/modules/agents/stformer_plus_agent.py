...
import torch
from modules.helpers.models.stformer_plus import STformerPlus
from modules.helpers.embedding.dummy_generator import DummyGenerator


class STformerPlusAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(STformerPlusAgent, self).__init__()
        self.args = args

        self.dummy_generator = DummyGenerator(args.device)
        
        self.transformer = STformerPlus(args, args.token_dim, args.emb)
    ...
    def forward(self, inputs, hidden_state, decoder_outs, task_enemy_num = None, task_ally_num = None, env = "sc2"):
        ...
        encoder_outs, decoder_outs, hidden = self.transformer.forward(encoder_inputs, decoder_inputs, hidden_state, decoder_outs, None)
        outs = torch.cat((encoder_outs, decoder_outs[:, :1, :]), -1)
