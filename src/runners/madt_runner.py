...
from utils import ShareSubprocVecEnv
...
class MADTRunner:
    ...
    def __init__(self, args, logger):
        ...
        self.eval_env = Env(args.eval_episodes)
    ...
    def setup(self, mac):
        ...
        # prepare the buffer
        used_data_dir = []
        for map_name in self.args.offline_map_lists:
            source_dir = self.offline_data_dir + map_name
            for quality in self.offline_data_quality:
                used_data_dir.append(f"{source_dir}/{quality}/")

        self.buffer.load_offline_data(used_data_dir, self.offline_episode_num, max_epi_length = self.eval_env.max_timestep)
